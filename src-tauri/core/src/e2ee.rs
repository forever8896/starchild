...
pub enum E2eeError {
    ...
    /// attestation fetch so this core type stays networking-free (WASM-safe).
    #[error("HTTP error: {0}")]
    Network(String),
...
impl E2eeSession {
    /// Build a session from the model's attested public key (the pure half of
    /// the handshake — no networking).
    ///
    ///   1. Generate a fresh ephemeral secp256k1 client key pair
    ///   2. Normalize + parse the model's signing public key
    ///
    /// The network half (fetching/verifying the TEE attestation that yields
    /// `model_pub_hex`) lives desktop-side in `e2ee::establish`, which calls
    /// this once it has the key.
    pub fn new(model_pub_hex: impl Into<String>) -> Result<Self> {
        ...
        // 2. Model's public key (secp256k1, uncompressed, 130 hex chars starting with 04)
        let mut model_pub_hex = model_pub_hex.into();
        ...
        log::info!(
            "E2EE session keys ready (client_pub={}...)",
            &client_pub_hex[..16]
        );
