...
impl StarchildState {
    ...
    /// Like [`new`](Self::new) but with the clock injected, so callers on
    /// platforms without `SystemTime` (WASM) can supply the timestamp.
    ...
    pub fn from_db_row(
        ...
        Self::from_db_row_at(Utc::now(), hunger, mood, energy, bond, xp, level, last_decay_at)
    }

    /// Like [`from_db_row`](Self::from_db_row) but with the parse-failure
    /// fallback clock injected (WASM-safe).
    #[allow(clippy::too_many_arguments)]
    pub fn from_db_row_at(
        now: DateTime<Utc>,
        hunger: f64,
        mood: &str,
        energy: f64,
    ...
    pub fn apply_hunger_decay(&mut self) {
        self.apply_hunger_decay_at(Utc::now());
    }

    /// Like [`apply_hunger_decay`](Self::apply_hunger_decay) but with the clock
    /// injected, so the elapsed-decay calc works without `SystemTime` (WASM).
    pub fn apply_hunger_decay_at(&mut self, now: DateTime<Utc>) {
